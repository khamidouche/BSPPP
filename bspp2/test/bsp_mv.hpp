...
#ifndef  BSP_MV_HPP__INCLUDED__
#define  BSP_MV_HPP__INCLUDED__
...
template<class T>
boost::optional<view<T> > transmi_all ( int ind,boost::optional<view<T> > & v)
  {
return v;
  }
...
void bsp_mv(int const& p, int const& s, par<int>&NA, par<int>& NZ, int const & pnrows,
           par<std::vector<T> >& A_I_J,
           std::vector<int>& prowindex,std::vector<int>& pcolindex,
           std::vector<double>& v,std::vector<double>& u_p,std::vector<double>& u,
           par<view<V> >& ParViewMat_U,V MAT_U){
    ...
    */
    ...
    parfun<view<V> > fun_all;
    fun_all=replicate(bl::bind(transmi_all<V>,bl::_1,bl::_2));
        ...
        for(k=0;k<pnrows;k++)
         {
           ...
           u_p[prowindex[k]]=u_p[prowindex[k]]+ A_I_J()[prowindex[k]].a * v[pcolindex[k]];
          }

        MAT_U[s]=u_p;

        put(ParViewMat_U,fun_all);
    }
    // summation of the u_p
    super_step()
    {
      for (j=0;j<NA();j++)
       for(i=0;i<p;i++)
    ...
    if(s==0)
     ...
     std::cout<<"u["<<i<<"] = "<<u[i]<<std::endl;
            ...
            //void bspmv_init(int const& p, int const& s, par<int>const&NA , int &nrows, int& ncols,
            //                //int nv, int nu,
            //
